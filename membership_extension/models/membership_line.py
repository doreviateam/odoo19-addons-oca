...
class MembershipLine(models.Model):
    ...
    def _compute_state(self):
        no_invoice_lines = self.filtered(
            # In Odoo 19, relying on NewId internals is fragile during onchange.
            # ``_origin.id`` stays falsy for unsaved x2many lines, which is enough here.
            lambda line: not line._origin.id or not line.account_invoice_id
        )
