...
class AccountReconcileModel(models.Model):
    ...
    # Odoo 19: match_text_location_* may be removed in core, add for OCA logic
    match_text_location_label = fields.Boolean(
        string="Match Text Location Label",
        default=True,
        help="Search in the Statement's Label to find the Invoice/Payment's reference",
    )
    match_text_location_note = fields.Boolean(
        string="Match Text Location Note",
        default=False,
        help="Search in the Statement's Note to find the Invoice/Payment's reference",
    )
    match_text_location_reference = fields.Boolean(
        string="Match Text Location Reference",
        default=False,
        help="Search in the Statement's Reference to find the Invoice/Payment's reference",
    )

    # Odoo 19: rule_type removed in core, provide compat for OCA logic
    is_writeoff_suggestion = fields.Boolean(
        string="Write-off suggestion",
        default=False,
        help="If set, model acts as writeoff_suggestion (suggest write-off only).",
    )
    ...
    @api.depends("trigger", "is_writeoff_suggestion")
    def _compute_rule_type(self):
        for rec in self:
            if rec.trigger == "manual":
                rec.rule_type = "writeoff_button"
            elif rec.is_writeoff_suggestion:
                rec.rule_type = "writeoff_suggestion"
            else:
    ...
    @api.model
    def _search(self, args, order=None, offset=0, limit=None, **kwargs):
        """Rewrite domain conditions on rule_type to trigger/is_writeoff_suggestion.

        Odoo 19 core removed rule_type; some code may still pass domains like
        ('rule_type', 'in', ['invoice_matching', 'writeoff_suggestion']).
        We translate those so search() does not raise Invalid field rule_type.
        Odoo 19: BaseModel._search() no longer accepts access_rights_uid.
        """
        args = self._domain_rule_type_to_trigger(args)
        return super()._search(args, order=order, offset=offset, limit=limit)

    @api.model
    def _domain_rule_type_to_trigger(self, domain):
        """Replace rule_type leaves by equivalent trigger / is_writeoff_suggestion."""
        if not domain:
            return domain
        result = []
        for item in domain:
            if isinstance(item, (list, tuple)):
                if len(item) == 3 and item[0] == "rule_type":
                    replacement = self._rule_type_leaf_to_trigger(item[1], item[2])
                    ...
                    result.append(self._domain_rule_type_to_trigger(item))
            else:
                result.append(item)
        return result

    @api.model
    def _rule_type_leaf_to_trigger(self, operator, value):
        """Convert one rule_type leaf to trigger/is_writeoff_suggestion leaves."""
        ...
        if operator in ("in", "="):
            leaves = []
            if "invoice_matching" in values or "writeoff_suggestion" in values:
                leaves.append(("trigger", "=", "auto_reconcile"))
                if set(values) == {"invoice_matching"}:
                    leaves.append(("is_writeoff_suggestion", "=", False))
                elif set(values) == {"writeoff_suggestion"}:
                    leaves.append(("is_writeoff_suggestion", "=", True))
            if "writeoff_button" in values:
                if leaves:
                    return [
                        ("|", ("trigger", "=", "auto_reconcile"), ("trigger", "=", "manual"))
                    ]
                leaves.append(("trigger", "=", "manual"))
            return leaves if leaves else [("id", "=", 0)]
        if operator in ("!=", "not in"):
            all_types = {"invoice_matching", "writeoff_suggestion", "writeoff_button"}
            excluded = set(values)
            included = list(all_types - excluded)
            if not included:
                return [("id", "=", 0)]
            return self._rule_type_leaf_to_trigger("in", included)
        return [("id", "=", 0)]
    ...
    def _get_write_off_move_lines_dict(self, residual_balance, partner_id, journal=None):
        ...
        :param journal: Optional journal for move lines (Odoo 19: line has no journal_id).
        :return: A list of dict representing move.lines to be created corresponding to
          the write-off lines.
        """
        self.ensure_one()
        ...
        if self.rule_type == "invoice_matching" and (
            not getattr(self, "allow_payment_tolerance", False)
            or getattr(self, "payment_tolerance_param", 0) == 0
        ):
            return []

        currency = self.company_id.currency_id
        match_journal_ids = getattr(self, "match_journal_ids", self.env["account.journal"])
        journal_id = (
            journal.id if journal
            else match_journal_ids[:1].id if match_journal_ids
        ...
        for line in self.line_ids:
            ...
            writeoff_line = line._get_write_off_move_line_dict(
                balance, currency, journal_id=journal_id
            )
            ...
            if line.tax_ids:
                taxes = line.tax_ids
                force_tax_included = getattr(line, "force_tax_included", False)
        ...
        for rec_model in available_models:
            ...
            if rec_model.rule_type == "invoice_matching":
                ...
                return {
                    "model": rec_model,
                    "status": "write_off",
                    "auto_reconcile": getattr(rec_model, "auto_reconcile", False),
                }
        return {}
    ...
    def _is_applicable_for(self, st_line, partner):
        ...
        # Odoo 19: some fields may not exist (match_nature, match_partner, etc.)
        match_nature = getattr(self, "match_nature", None)
        match_amount = getattr(self, "match_amount", None)
        match_partner = getattr(self, "match_partner", False)
        match_partner_ids = getattr(self, "match_partner_ids", self.env["res.partner"])
        match_partner_category_ids = getattr(
            self, "match_partner_category_ids", self.env["res.partner.category"]
        )
        st_line_journal = st_line.move_id.journal_id if st_line.move_id else st_line.journal_id
        match_journal_ids = getattr(self, "match_journal_ids", self.env["account.journal"])
        if (
            (
                match_journal_ids
                and st_line_journal not in match_journal_ids
            )
            or (match_nature == "amount_received" and st_line.amount < 0)
            or (match_nature == "amount_paid" and st_line.amount > 0)
            or (
                match_amount == "lower"
                and abs(st_line.amount) >= getattr(self, "match_amount_max", 0)
            )
            or (
                match_amount == "greater"
                and abs(st_line.amount) <= getattr(self, "match_amount_min", 0)
            )
            or (
                match_amount == "between"
                and (
                    abs(st_line.amount) > getattr(self, "match_amount_max", 0)
                    or abs(st_line.amount) < getattr(self, "match_amount_min", 0)
                )
            )
            or (match_partner and not partner)
            or (
                match_partner
                and match_partner_ids
                and partner not in match_partner_ids
            )
            or (
                match_partner
                and match_partner_category_ids
                and not (partner.category_id & match_partner_category_ids)
            )
        ):
            return False

        # Filter on label, note and transaction_type
        # Odoo 19: some match_* fields may not exist
        for record, rule_field, record_field in [
            ...
            if record is None:
                continue
            param_field = "match_" + rule_field + "_param"
            match_field = "match_" + rule_field
            if param_field not in self._fields or match_field not in self._fields:
                continue
            rule_term = (getattr(self, param_field) or "").lower()
            record_term = (record[record_field] or "").lower()

            # This defines non-match conditions
            match_val = getattr(self, match_field, None)
            if (
                (match_val == "contains" and rule_term not in record_term)
                or (match_val == "not_contains" and rule_term in record_term)
                or (
    ...
    def _get_invoice_matching_amls_domain(self, st_line, partner):
        ...
        if getattr(self, "match_same_currency", True):
            aml_domain.append(("currency_id", "=", currency.id))

        if partner:
            aml_domain.append(("partner_id", "=", partner.id))

        past_months_limit = getattr(self, "past_months_limit", 0)
    ...
    def _get_invoice_matching_amls_candidates(self, st_line, partner):
        ...
        def get_order_by_clause(alias=None):
            matching_order = getattr(self, "matching_order", "new_first")
            direction = "DESC" if matching_order == "new_first" else "ASC"
        ...
        # Odoo 19: _where_calc may be removed, use fallback
        try:
            query = aml_model._where_calc(aml_domain)
        ...
        if from_clause and where_clause and (numerical_tokens or exact_tokens):
            aml_cte = rf"""
                WITH aml_cte AS (
                    SELECT
        ...
        if not partner:
            ...
            if from_clause and where_clause:
                ...
                # Odoo 19 fallback: use search when _where_calc unavailable
                amls = aml_model.search(
                    aml_domain + [("currency_id", "=", st_line_currency.id)],
                    order=get_order_by_clause(),
                )
                target = -st_line.amount_residual
                if st_line_currency == self.company_id.currency_id:
                    amls = amls.filtered(
                        lambda a: st_line_currency.is_zero(
                            a.amount_residual - target
                        )
                    )
                else:
                    amls = amls.filtered(
                        lambda a: st_line_currency.is_zero(
                            a.amount_residual_currency - target
                        )
                    )
        else:
    ...
    def _get_partner_from_mapping(self, st_line):
        ...
        # Odoo 19: rule_type removed, use trigger - only auto_reconcile does mapping
        if self.trigger != "auto_reconcile":
            return self.env["res.partner"]

        # Odoo 19: partner_mapping_line_ids may not exist in core
        if "partner_mapping_line_ids" not in self._fields:
            return self.env["res.partner"]
    ...
    def _get_invoice_matching_amls_result(self, st_line, partner, candidate_vals):  # noqa: C901
        def _create_result_dict(amls_values_list, status):
            ...
            if (
                ...
                and getattr(self, "auto_reconcile", False)
            ):
    ...
    def _check_rule_propositions(self, st_line, amls_values_list):
        ...
        if not getattr(self, "allow_payment_tolerance", False):
        ...
        if getattr(self, "payment_tolerance_param", 0) == 0:
            return {"rejected"}

        # If the tolerance is expressed as a fixed amount, check the residual payment
        # amount doesn't exceed the tolerance.
        if (
            getattr(self, "payment_tolerance_type", "percentage") == "fixed_amount"
            and st_line_currency.compare_amounts(
                -amount_curr_after_rec, getattr(self, "payment_tolerance_param", 0)
            )
            <= 0
        ):
            return {"allow_write_off", "allow_auto_reconcile"}
        ...
        if (
            getattr(self, "payment_tolerance_type", "percentage") == "percentage"
            and st_line_currency.compare_amounts(
                reconciled_percentage_left, getattr(self, "payment_tolerance_param", 0)
            )
            <= 0
        ):
            return {"allow_write_off", "allow_auto_reconcile"}
...
class AccountReconcileModelLine(models.Model):
    ...
    def _get_write_off_move_line_dict(self, balance, currency, journal_id=False):
        """Odoo 19: journal_id passed from caller (line has no journal_id)."""
        self.ensure_one()
        return {
            "name": self.label,
            "balance": balance,
