...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def _get_domain_by_inheritance(self, operation):
        ...
        return list(fields.Domain.AND([inherited_access_domain, fields.Domain.OR(domains)]))
    ...
    def _get_permission_domain(self, operator, value, operation):
        ...
        positive = (operator not in fields.Domain.NEGATIVE_OPERATORS) == bool(value)
        if _self.env.su:
            # You're SUPERUSER_ID
            return list(fields.Domain.TRUE if positive else fields.Domain.FALSE)

        result = fields.Domain.OR(
            [
